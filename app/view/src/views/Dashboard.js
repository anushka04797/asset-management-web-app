    ...
    const exportToCSV = (csvData) => {
      ...
      for (let i = 0; i < csvData.length; i++) {
       ...
       // assigneName = assigneNames.join(",");
        xlData.push({ 'Sl. No': i + 1, 'Serial No.': item.ast_serial, 'Asset Name': item.ast_name, 'Asset type': item.ast_type.ast_title, 'River': item.ast_river.riv_name, 'Address': item.ast_address?item.ast_address+', ' + item.ast_union.uni_name+', '+item.ast_upazila.upa_name+', '+item.ast_district.dis_name+', '+item.ast_division.div_name :
  ...
  useEffect(() => {
    axios
      ...
      .then(function(response) {
        ...
        console.log('res',response.data.data[0].ast_river.riv_name)
        console.log('date',response.data.data[0].ast_updated_at?response.data.data[0].ast_updated_at:response.data.data[0].ast_created_at)
      })
            ...
            <Card className="table-holder-class">
              ...
              <CardBody>
                ...
                <Dropdown isOpen={isOpen} toggle={toggleDropDown} className="pull-right mr-1" size="sm">
                    <DropdownToggle><i class="fa fa-calendar mr-2"></i>{selected_option} <i class="fas fa-caret-down"></i></DropdownToggle>
                    <DropdownMenu>
                  ...
                  <tbody>
                    {assets.map((item, idx) => (
                      ...
                      <tr key={idx}>
                        ...
                        <td className="text-center pond-actual-value">{item.ast_address?item.ast_address+', ' + item.ast_union.uni_name+', '+item.ast_upazila.upa_name+', '+item.ast_district.dis_name+', '+item.ast_division.div_name :
                        item.ast_union.uni_name+', '+item.ast_upazila.upa_name+', '+item.ast_district.dis_name+', '+item.ast_division.div_name}</td>
